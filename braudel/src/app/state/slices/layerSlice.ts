// app/state/slices/layerSlice.ts
...
export function handleAddLayer(state: any, name: string, type: 'physical' | 'political' | 'economic' | 'social' | 'cultural' | 'historical') {
  ...
  const newLayer = createLayer(worldId, type as any, name, state.world.layers.length);
  db.put('layers', newLayer);
  return {
    world: {
      ...state.world,
